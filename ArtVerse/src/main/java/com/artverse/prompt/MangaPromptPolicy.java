...
public final class MangaPromptPolicy {
    ...
    private static final Pattern DIALOGUE_SPEAKER_PATTERN =
            Pattern.compile("(对话气泡：「)[^」：:]*[：:]\\s*");
    ...
    public static String storyboardInstruction(int pageCount) {
        return """
                ...
                - 对白格式为：对话气泡：「台词内容」（气泡内严禁写入角色名字、说话人标识，仅保留纯台词文本）。旁白格式为：旁白框：「内容」。
                ...
                示例格式：
                [
                  "第1页：【第1格（远景）】... 对话气泡：「简体中文台词」。【第2格（近景）】... 音效字：唰。【第3格（特写）】...【第4格（中景）】...",
    ...
    public static String buildImagePrompt(String scene, String profiles, String mangaStyle, String colorMode,
        ...
        sb.append("- 整体画布无外围白色边框、无多余白边底色，整体底色与漫画内容融为一体，仅保留漫画内部分格边框\n");
        sb.append("- 对话气泡内只保留纯台词文本，严禁添加角色名、说话人标签\n\n");
        ...
        if (allScenes != null && !allScenes.isEmpty()) {
            for (int i = 0; i < allScenes.size(); i++) {
                sb.append("第").append(i + 1).append("页：").append(sanitizeScene(allScenes.get(i))).append("\n");
            }
        } else {
        ...
        sb.append("- 竖向漫画页构图，页面内必须有 4-6 个清晰分镜格，格子之间有明确边框。")
                .append("整页画布禁止外围整体白框、外留白边、额外白色底色，仅保留漫画内部格子边框，整体底色和漫画画面融为一体。\n");
        sb.append("- 每格画面要承接剧情动作，形成连续叙事；不要把人物摆成单幅宣传海报。\n");
        sb.append("- 保留分镜中的对白气泡和音效字，但所有可读文字必须是简体中文。\n");
        sb.append("- 对话气泡：仅放置纯台词文本，绝对不能包含角色名称、说话者名字、角色标识；")
                .append("文字短而清晰；禁止英文、繁体字、乱码、伪文字、罗马字。\n");
    ...
    private static String styleTemplate(String mangaStyle) {
        return switch (mangaStyle == null ? "japanese_manga" : mangaStyle) {
            case "korean_webtoon" -> "韩式条漫气质，干净线条，自然渐变光影，角色比例修长，适合竖屏阅读，但当前输出仍必须是一页多格漫画，无整体外围白框。";
            case "american_comic" -> "美式漫画风格，粗重有力的线条，高对比阴影，动态透视强，动作冲击力强；拟声词仍必须使用简体中文，无整体外围白框。";
            case "ligne_claire" -> "欧式清线漫画风格，线条均匀清晰，平涂色块，背景细节明确，叙事清楚，无整体外围白框。";
            case "chinese_ink" -> "中国水墨国风漫画，水墨渲染、工笔线条、留白构图和传统场景元素结合，但分镜边框和阅读顺序必须清晰，无整体外围白框。";
            case "semi_realistic" -> "半厚涂写实漫画风格，角色细腻、材质明确、光影自然，同时保留漫画分格和气泡，无整体外围白框。";
            case "realistic" -> "写实漫画风格，真实光影和人物比例，电影感构图，但必须保持多格漫画页结构，无整体外围白框。";
            case "oil_painting" -> "厚涂油画漫画风格，笔触丰富、色彩层次强，同时保留清晰漫画分格，无整体外围白框。";
            case "flat_design" -> "扁平极简漫画风格，形状清楚、色块干净、信息可读，分镜结构明确，无整体外围白框。";
            case "pixel_art" -> "像素风漫画，像素颗粒清楚，复古游戏感，但仍是多格漫画页，无整体外围白框。";
            case "watercolor" -> "水彩淡雅漫画风格，柔和透明的色彩和纸张质感，分镜边界清晰，无整体外围白框。";
            case "cyberpunk" -> "赛博朋克漫画风格，霓虹光、城市科技感、高对比色彩，但文字必须为简体中文，无整体外围白框。";
            default -> "日式漫画风格，精细线条、网点阴影、戏剧性光影、清晰多格分镜、表情和动作生动，无整体外围白框。";
        };
    }

    private static String colorModifier(String colorMode) {
        return switch (colorMode == null ? "bw" : colorMode) {
            case "color" -> "色彩模式：全彩，高饱和但不脏乱，角色和背景层次分明，无整体外围白框。";
            case "grayscale" -> "色彩模式：灰度，保留丰富中间灰阶和素描质感，无整体外围白框。";
            case "duotone" -> "色彩模式：双色调，使用有限色彩制造强烈情绪氛围，无整体外围白框。";
            default -> "色彩模式：黑白，高对比线稿、网点纸纹、清晰明暗层次，无整体外围白框。";
        };
    }

    /**
     * Strip speaker name prefixes from dialogue bubbles in scene text.
     * Converts {@code 对话气泡：「角色名：台词」} to {@code 对话气泡：「台词」}
     * as a defense-in-depth measure for storyboards generated before the
     * no-speaker-name rule was enforced.
     */
    static String sanitizeScene(String scene) {
        if (scene == null || scene.isBlank()) {
            return scene;
        }
        return DIALOGUE_SPEAKER_PATTERN.matcher(scene).replaceAll("$1");
    }
}
