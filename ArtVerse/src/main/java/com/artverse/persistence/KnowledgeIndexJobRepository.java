...
public interface KnowledgeIndexJobRepository extends JpaRepository<KnowledgeIndexJob, Long> {
    ...
    @Query("SELECT j FROM KnowledgeIndexJob j WHERE j.status IN :statuses AND j.nextRunAt <= :now "
            + "AND (j.status <> com.artverse.domain.KnowledgeIndexJobStatus.FAILED OR j.attempts < 5) "
            + "AND (j.status <> com.artverse.domain.KnowledgeIndexJobStatus.RUNNING "
            + "OR j.leaseUntil IS NULL OR j.leaseUntil <= :now) ORDER BY j.createdAt ASC")
