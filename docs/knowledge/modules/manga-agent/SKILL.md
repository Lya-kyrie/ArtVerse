...
- Runtime Skill registry: `ArtVerseSkillRegistry` and AgentScope `ArtVerseSkillRepository`; only published platform Skills are loaded.
...
`AgentScopeAgentFactory` creates or reuses a per-user/story/chapter/conversation/task/model/workspace/Skill-version agent and delegates AgentScope v2 `Toolkit` setup to the strategy registered for the request's task type. It attaches the read-only `ArtVerseSkillRepository`, disables dynamic/default workspace Skills, and enables only the published Skill selected by application capability. `AgentScopeRuntimeContextFactory` passes server-owned identity, step, request, and fencing values through AgentScope v2 `RuntimeContext` as `MangaAgentRuntimeContext`.
