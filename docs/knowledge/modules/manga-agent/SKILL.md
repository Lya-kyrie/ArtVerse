...
- AgentScope event mapping and Director request helpers: `ArtVerse/src/main/java/com/artverse/application/workflow/nodes/MangaDirectorAgentSupport.java`.
...
For a new run, `MangaAgentService` resolves the active `MangaAgentConversation` unless a conversation id is supplied, opens the per-run tool tracking scope, and delegates workflow execution to `MangaWorkflowOrchestrator`. The orchestrator validates the message, starts or reuses the run, checks idempotency through `GenerationGuardService.executeMangaAgentRun`, assembles a workflow context snapshot, and dispatches the explicit `DIRECTOR` route through `MangaWorkflowNodeRegistry`.

`MangaDirectorAgentNode` is the current concrete AgentScope-backed node. It keeps AgentScope invocation and final response handling, while `MangaDirectorAgentSupport` saves the user message, builds agent messages from the selected conversation history, syncs chapter knowledge to the AgentScope workspace, builds an `AgentRunRequest`, maps streamed events, and saves the final assistant or degraded reply.
...
The frontend consumes AG-UI as the default live protocol. `POST /conversations/{conversationId}/ag-ui/run` and `POST /conversations/{conversationId}/ag-ui/runs/{requestId}/resume` are the preferred endpoints. Runtime execution currently uses the `DIRECTOR` route only; route values in persisted snapshots are metadata, not a frontend mode selector. Legacy chapter-level endpoints auto-resolve the active conversation and remain compatibility paths. Keep the execution panel as the single place that explains what the agent is doing; do not add a second competing progress widget.
...
- Workflow route is an explicit application contract. `DIRECTOR` is the only registered route today, and missing handlers must fail fast instead of silently falling back to `DIRECTOR`.
